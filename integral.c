...
double find_integral(double l, double r, double (*func)(double))
{
    ...
    int n = ceil(len * len * len * MAX_DERIV / EPS2 / 2); // предполагается f'(x) = 1.0 в каком-то приближении
...
double find_integral(double l, double r, double (*func)(double))
{
    ...
    int n = ceil(sqrt(len * len * len * MAX_DERIV / EPS2 / 12)); // предполагается f''(x) = 1.0 в каком-то приближении
...
double find_integral(double l, double r, double (*func)(double))
{
    ...
    int n = ceil(pow(len2 * len2 * len * MAX_DERIV / EPS2 / 2880.0, 0.25)); // предполагается f''''(x) = 1.0 в каком-то приближении
    n += n % 2;
